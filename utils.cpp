...
set<Point> enum_nearby(const set<Point>& ps)//枚举一组点,返回和这一组点相邻的所有点（已经排除它们本身）
{
    set<Point> ret;
    for (auto it = ps.begin(); it != ps.end(); ++it)
    {
        ret=ret+enum_nearby(*it);
    }
    ret=ret-ps;//去除本身
    return ret;
}
set<Point> enum_nearby_all(const set<Point> &ps) // 枚举一组点,返回和这一组点相邻的所有点（包括它们本身）
{
    set<Point> ret;
    for (auto it = ps.begin(); it != ps.end(); ++it)
    {
